...
<a class="card" href="product.php?id=<?= (int)$p['id'] ?>">
    <div class="card-img">
        ...
        <?php if (!empty($p['old_price']) && (float)$p['old_price'] > (float)$p['price']): ?>
            <span class="off"><?= fa_num(round(((float)$p['old_price'] - (float)$p['price']) / (float)$p['old_price'] * 100)) ?>٪ تخفیف</span>
        <?php endif; ?>
    ...
    <div class="card-body">
        <h3><?= e($p['name']) ?></h3>
        <div class="card-meta">
            <?php if ($p['color']): ?><span><?= icon('palette') ?> <?= e($p['color']) ?></span><?php endif; ?>
            <?php if ($p['width_cm']): ?><span><?= icon('ruler') ?> عرض <?= fa_num($p['width_cm']) ?></span><?php endif; ?>
        </div>
        <div class="card-foot">
            <div class="card-price">
                ...
                <?php endif; ?>
                <div class="cp-main"><b><?= fa_num(number_format((float)$p['price'])) ?></b><i>تومان / متر</i></div>
            </div>
            <span class="card-go"><?= icon('arrow-left') ?></span>
        </div>
    </div>
</a>
